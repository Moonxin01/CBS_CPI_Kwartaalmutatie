...
# Data verwerken
verwerk_data <- function(raw, verslagperiode = NULL) {
  ...
  if (!is.null(verslagperiode)) {
    if (nchar(verslagperiode) == 4) {
      verwerkt <- verwerkt |> filter(jaar == as.integer(verslagperiode))
    } else {
      jaar_filter <- as.integer(substr(verslagperiode, 1, 4))
      kw_filter   <- as.integer(substr(verslagperiode, 7, 8))
      verwerkt <- verwerkt |> filter(jaar > jaar_filter |
...
maak_grafieken <- function(data) {
  ...
  # Lijngrafiek
  lijn <- ggplot(data |> filter(jaar >= (max(jaar) - 4)),
                 aes(x = periode_label, y = kwartaalmutatie_pct,
    ...
    labs(title    = "Kwartaalmutatie consumentenprijsindex",
         subtitle = "Huur van woning en Benzine | Bron: CBS StatLine (83131NED)",
  ...
  # Staafdiagram
  staaf <- ggplot(data |> group_by(categorie_label) |> slice_tail(n = 8) |> ungroup(),
                  aes(x = periode_label, y = kwartaalmutatie_pct,
  ...
  ggsave("output/grafiek1_lijngrafiek.png",  lijn, width = 10, height = 5.5, dpi = 150)
  ggsave("output/grafiek2_staafdiagram.png", staaf, width = 10, height = 5.5, dpi = 150)
}
...
main <- function(verslagperiode = NULL) {
  ...
  resultaat <- haal_data_op() |>
    verwerk_data(verslagperiode)
  
  maak_grafieken(resultaat)
